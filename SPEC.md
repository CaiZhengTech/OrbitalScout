...
**Bin width.** The bin width is 200 GDD, and a baseline cell is used only if it rests on at least three prior years. This replaces a rule fixed before binning, which chose the narrowest of 150, 200, 250 and 300 GDD leaving at least 90% of zone-year-bins observed; on real data that rule selected 300 GDD, the width the Step 2 decision record had already named as too coarse to resolve growth stages. It was retired rather than followed because widening bins adds no observations to the sparse window, late May into June at 400 to 800 GDD, and only enlarges cells until the coverage figure passes, so the rule could be satisfied without improving the baseline support it stood in for. Width is now chosen for resolution, four bins across emergence to canopy closure, and support is enforced directly by the prior-year floor, applied where the baseline is consumed. A field-date is used only if at least half the field's zones are clear, because the field median is taken over visible zones and cloud is contiguous. Evidence in `RESULTS.md`; reasoning in `docs/reviews/2026-09-17-step2-decisions.md`, amendment Decisions 5 and 6.
...
DuckDB over Parquet, local. **Measured after Step 1: 149,572,812 zone-date rows across 8 seasons, 1.65 GB of Parquet.** That is the order of magnitude this section predicted; the earlier estimate of 180 million assumed unbuffered fields, and the inward buffer of Section 6 removes roughly a quarter of each field's area.

`zone_obs` is a **view** over the Parquet, not a table. The Parquet files are the storage and the database holds only `fields` and `zones`, which is 7.4 MB against the 1.65 GB it indexes. Materialising the view copies every row into the database for nothing, and at eight seasons that was enough for the OS to kill the load. Two tests assert the view stays a view, because the mistake is invisible at one season. This claim is conditional on 30m and is not true at 10m, where the same AOI would be about 1.6 billion rows and roughly 25 GB. The two were inconsistent in earlier drafts.
